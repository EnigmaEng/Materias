...
const {crearPlato, mensaje} = TodoContext
...
const formik = useFormik({
  ...
  costo: Yup.string().required('El campo no puede ir vacío'),
...
onSubmit: (valores, {resetForm}) => {
  ...
  const platoData = {
    ...
    estado_plato: valores.estado_plato
  }
...
<div className='flex '>
  <input type="number" id='costo' value={formik.values.costo} onChange={formik.handleChange} onBlur={formik.handleBlur} className='appearance-none w-80 ring-2 ring-wwe border border-wwe py-2 px-4 border border-gray-300 rounded-lg leading-5 ml-14 transition duration-150 ease-in-out sm:text-sm sm:leading-5 custom-number-input bg-white' placeholder='Precio..'/>
<span className='relative right-16 text-white text-lg py-2 px-7 bg-wwe w-16 rounded-md'>$</span>
</div>

        </div>
         <div className='flex flex-col'>
<label htmlFor="">Estado del plato:</label> 
<select name="" id="estado_plato" className='select w-80 bg-white border border-wwe ring-2 ring-wwe' value={formik.values.estado_plato} onChange={formik.handleChange} onBlur={formik.handleBlur} >
